...
const UnityMaker = artifacts.require('UnityMaker');
const MockBEP20 = artifacts.require('libs/MockBEP20');

contract('UnityMaker', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.unity = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.chef = await UnityMaker.new(this.unity.address, '40', '300', '400', {
      from: minter,
    });
  });

  it('unity chef now', async () => {
    await this.unity.transfer(bob, '1000', { from: minter });
    await this.unity.transfer(carol, '1000', { from: minter });
    await this.unity.transfer(alice, '1000', { from: minter });
    assert.equal((await this.unity.balanceOf(bob)).toString(), '1000');

    await this.unity.approve(this.chef.address, '1000', { from: bob });
    await this.unity.approve(this.chef.address, '1000', { from: alice });
    await this.unity.approve(this.chef.address, '1000', { from: carol });

    await this.chef.deposit('10', { from: bob });
    assert.equal(
      (await this.unity.balanceOf(this.chef.address)).toString(),
      '10'
    );
  ...
  it('try unity', async () => {
    ...
    this.chef2 = await UnityMaker.new(this.unity.address, '40', '600', '800', {
      from: minter,
    });
    await this.unity.approve(this.chef2.address, '10', { from: alice });
    await time.advanceBlockTo('590');
    this.chef2.deposit('10', { from: alice }); //520
    await time.advanceBlockTo('610');
    assert.equal(
      (await this.unity.balanceOf(this.chef2.address)).toString(),
      '10'
    );
    assert.equal(
  ...
  it('emergencyWithdraw', async () => {
    ...
    assert.equal((await this.unity.balanceOf(alice)).toString(), '990');
